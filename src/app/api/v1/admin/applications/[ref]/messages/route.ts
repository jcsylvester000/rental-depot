...
/** POST /api/v1/admin/applications/:ref/messages — operator replies (gated by the chat invitation). */
export async function POST(req: NextRequest, { params }: { params: Promise<{ ref: string }> }) {
  try {
    ...
    const result = await store.addMessage(decodeURIComponent(ref), body, "operator");
    if (!result.ok) {
