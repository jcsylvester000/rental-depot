...
/** POST /api/v1/applications/:ref/chat — applicant accepts or declines a property chat request. */
export async function POST(req: NextRequest, { params }: { params: Promise<{ ref: string }> }) {
  try {
    ...
    const app = action === "accept"
      ? await store.acceptChat(decodeURIComponent(ref), "applicant")
      : await store.declineChat(decodeURIComponent(ref), "applicant");
